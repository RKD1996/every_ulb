    ...
    <v-card>
      <v-card-text>
        <v-layout row wrap>
          <v-flex xs12 sm6>
            <v-stepper vertical>
              <div v-for="(t,i) in planningData"  :key="t.id">
                <v-stepper-step :complete="t.status" :step="t.id"></v-stepper-step>
                <v-stepper-content :step="t.id">
                  <v-card :color="colors" @click.native="showPlan(t.topic, t.critical, t.eoi, t.final)">
                    <v-card-text>
                      <v-layout row wrap>
          ...
          <v-flex xs12 sm6>
            <v-card class="ma-4" color="yellow lighten-3" v-show="drop1 === 'plan'">
              <v-card-title >
...
export default {
  ...
  methods: {
    showPlan (topic, critical, eoi, final) {
      this.drop1 = 'plan'
